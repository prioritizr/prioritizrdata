# prioritizrdata: Conservation Planning Datasets

The prioritizrdata package is a supplemental package that contains
example datasets for conservation planning. It is intended to be used
alongside the prioritizr package.

## Details

This package contains the following datasets:

- [tas_data](http://prioritizr.github.io/prioritizrdata/reference/tas_data.md):

  Conservation planning dataset for Tasmania, Australia.

- [salt_data](http://prioritizr.github.io/prioritizrdata/reference/salt_data.md):

  Conservation planning dataset for Salt Spring Island, Canada.

- [wa_data](http://prioritizr.github.io/prioritizrdata/reference/wa_data.md):

  Conservation planning dataset for Washington, The United States of
  America.

## See also

Useful links:

- <https://prioritizr.github.io/prioritizrdata/>

- <https://github.com/prioritizr/prioritizrdata>

- Report bugs at <https://github.com/prioritizr/prioritizrdata/issues>

## Author

**Maintainer**: Richard Schuster <richard.schuster@glel.carleton.ca>
([ORCID](https://orcid.org/0000-0003-3191-7869))

Authors:

- Jeffrey O Hanson <jeffrey.hanson@uqconnect.edu.au>
  ([ORCID](https://orcid.org/0000-0002-4716-6134))

- Nina Morrell <nina.morrell@ubc.ca>

- Matthew Strimas-Mackey <mstrimas@gmail.com>
  ([ORCID](https://orcid.org/0000-0001-8929-7776))

- Kristin Camille V Buenafe <k.buenafe@uq.edu.au>
  ([ORCID](https://orcid.org/0000-0002-1643-5557))

- Matthew E Watts <m.watts@uq.edu.au>

- Peter Arcese <peter.arcese@ubc.ca>
  ([ORCID](https://orcid.org/0000-0002-8097-482X))

- Joseph R Bennett <joseph.bennett@carleton.ca>
  ([ORCID](https://orcid.org/0000-0002-3901-9513))

- Hugh P Possingham <hugh.possingham@tnc.org>
  ([ORCID](https://orcid.org/0000-0001-7755-996X))
